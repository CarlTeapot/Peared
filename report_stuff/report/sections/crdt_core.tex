...
სისტემის გულია \texttt{crdt-core}, Rust-ზე დაწერილი დამოუკიდებელი
ბიბლიოთეკა, რომელიც YATA-ს სტილის ტექსტურ CRDT-ს ახორციელებს. ის არაფერს
იცის არც ქსელის, არც ინტერფეისის შესახებ: მისი პასუხისმგებლობაა დოკუმენტის
მდგომარეობა, ოპერაციების ინტეგრაცია და ბინარული (wire) ფორმატი.

\paragraph{ბლოკები და დაკავშირებული სია.}
დოკუმენტი წარმოდგენილია \emph{ბლოკების} ორმაგად დაკავშირებულ სიად. ბლოკი
...
\begin{itemize}[itemsep=0pt]
  \item უნიკალური იდენტიფიკატორი \texttt{BlockId = (client\_id, clock)} -
        ...
        ანალოგი). გრძელი ბლოკი იდენტიფიკატორთა მთელ დიაპაზონს ფარავს:
        $n$ სიმბოლოიანი ბლოკი \texttt{(c, k)}-დან \texttt{(c, k+n-1)}-მდე;
  \item წარმოშობის მაჩვენებლები \texttt{origin\_left} და
        \texttt{origin\_right}, რომელი ორი ელემენტის შუაში დაიბადა ბლოკი
        ჩასმის მომენტში;
  \item შიგთავსი და წაშლის ნიშანი (\texttt{deleted}).
\end{itemize}

მთელი ტექსტის სიმბოლო-სიმბოლო შენახვის ნაცვლად ბლოკებით ოპერირება
...
ბლოკს ეწებება და ერთ ოპერაციად რჩება. ფაილის გახსნისას ტექსტი
64-სიმბოლოიან ბლოკებად იჭრება (\texttt{from\_text\_chunked}), რაც კარგი
შუალედია „ბევრ პატარა ობიექტსა“ და „ერთ გიგანტურ სტრიქონს“ შორის.
...
(\texttt{(c, k+offset)}). რადგან იდენტიფიკატორები დიაპაზონულია, გაყოფა
„უფასოა“, არცერთი არსებული მიმართვა არ ტყდება: ნებისმიერი
...
კრიტიკულია: დაშორებული მონაწილის ოპერაცია შეიძლება ისეთ ბლოკს
მიემართებოდეს, რომელიც ჩვენთან უკვე სამ ნაწილად არის გაყოფილი.

\paragraph{ინტეგრაცია და კონფლიქტების გადაწყვეტა.}
დაშორებული ბლოკის მიღებისას ალგორითმმა უნდა იპოვოს მისი ზუსტი ადგილი
...
(ორივე ბლოკს ერთი და იგივე origin-ები აქვს), YATA წესრიგს დეტერმინისტული
...
ჩანაწერები ერთად რჩება, ერთმანეთში არ ეწნება. Rust-ის ტიპთა სისტემა აქ
ძალიან დაგვეხმარა: ინტეგრაციის ლოგიკა მკაცრად გამიჯნულ მოდულებშია
(\texttt{document/integrate.rs}) და ყოველი უცნაური შემთხვევა ცალკე
ტესტითაა დაფარული.

\paragraph{წაშლა, DeleteSet და მდგომარეობის ვექტორი.}
...
რჩება სიაში, როგორც „საფლავის ქვა“ (tombstone), ტექსტში კი აღარ ჩანს.
წაშლები კომპაქტურად ინახება \texttt{DeleteSet} სტრუქტურაში, თითოეული
კლიენტისთვის წაშლილი \texttt{clock}-დიაპაზონების სიად. ეს წარმოდგენა
ორმაგად გამოსადეგია: ქსელით გადასაცემად მცირეა (ათასი ზედიზედ წაშლილი
სიმბოლო ერთი დიაპაზონია) და იდემპოტენტურია, ერთი და იმავე DeleteSet-ის
ორჯერ მიღება არაფერს აფუჭებს.

დოკუმენტის „სად ვარ“ მდგომარეობას აღწერს \emph{მდგომარეობის ვექტორი}
(state vector): თითო კლიენტზე, მისგან ნანახი ოპერაციების მაქსიმალური
\texttt{clock}. ორი რეპლიკის ვექტორების შედარებით ზუსტად ჩანს, რომელ
ოპერაციებს აკლია რომელიმე მხარეს. ჩვენ ვექტორებს ორ რამეში ვიყენებთ:
გვიან შემოსული სტუმრის სინქრონიზაციასა და ნაგვის შემგროვებელში (იხ.
...
დოკუმენტის სრული მდგომარეობა, ბლოკები, DeleteSet, მდგომარეობის ვექტორი -
...
საკონტროლო ფრეიმი (\texttt{0x02}), ნაგვის შემგროვებლის commit
(\texttt{0x04}), წევრობის ცვლილება (\texttt{0x05}), მდგომარეობის ვექტორის
...
ტესტები, ბაიტ-ბაიტ დაფიქსირებული ნიმუშები, რომლებიც მაშინვე გატყდება,
თუ ვინმემ ფორმატი ცალმხრივად შეცვალა.
...
დაკავშირებული სიის გავლით $O(n)$ ოპერაციაა და ყოველ კლავიშზე მთელი
...
ფოთლებში დოკუმენტისავე თანმიმდევრობით აწყვია, ხოლო ყოველი შიდა კვანძი
ინახავს ქვეხის ხილული სიმბოლოების ჯამურ სიგრძეს (\texttt{visible\_len};
...
ჯამებს ვაგროვებთ. ჩასმა, გაყოფა და წაშლა ხეში სარკისებურად აისახება და
დელტა ფესვამდე „ბუშტდება“. დიზაინის შთაგონება diamond-types პროექტიდან
\cite{diamond} მოვიდა.

მნიშვნელოვანი დეტალი: ხე \emph{მეორეული, წარმოებული} სტრუქტურაა -
ჭეშმარიტების წყარო ყოველთვის დაკავშირებული სია რჩება. იმისთვის, რომ ისინი
არასდროს დაშორდნენ, debug რეჟიმში ყოველი მუტაციის შემდეგ ეშვება „ორაკული“:
ფუნქცია, რომელიც მთელ სიას ხელით ჩამოუვლის და ამოწმებს, რომ ხის პასუხები
ზუსტად ემთხვევა, შეუსაბამობისას პროგრამა მაშინვე ვარდება panic-ით. ამან
განვითარების პროცესში არაერთი ფაქიზი შეცდომა დაიჭირა, ხოლო release
ბილდში ეს შემოწმება საერთოდ არ კომპილირდება და წარმადობას არ აზარალებს.
ხის განშტოების ფაქტორებიც განზრახ განსხვავებულია: debug-ში მცირეა (4),
რომ ტესტებში გაყოფის ლოგიკა ხშირად ვარჯიშობდეს, release-ში კი ფართო
(64/32), ნაკლები დონე და უკეთესი კეშ-ლოკალურობა.
...
\begin{diagram}[htbp]
  ...
  \end{tikzpicture}
  \caption{CRDT დოკუმენტის სტრუქტურა: ბლოკების დაკავშირებული სია
...
\begin{diagram}[htbp]
  ...
  დაკავშირებულ სიას და ხის ფოთოლს, შემდეგ კი ხილული სიგრძის დელტა
  ფესვამდე ვრცელდება}
  \label{diag:insert}
\end{diagram}
