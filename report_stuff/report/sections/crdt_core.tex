...
დოკუმენტი წარმოდგენილია \emph{ბლოკების} doubly linked list-ის ბლოკებად. ბლოკი
...
\begin{itemize}[itemsep=0pt]
  \item უნიკალური იდენტიფიკატორი \texttt{BlockId = (client\_id, clock)} -
        ...
        ანალოგი).
        $n$ სიმბოლოიანი ბლოკი არი \texttt{(c, k)}-დან \texttt{(c, k+n-1)}-მდე;
...
64 ბლოკებად იჭრება (\texttt{from\_text\_chunked}).
...
(\texttt{(c, k+offset)}). რადგან იდენტიფიკატორები სინამდვიელში დიაპაზონებზეა
...
კრიტიკულია: remote მონაწილის ოპერაცია შეიძლება ისეთ ბლოკს-ზე განხორციელდეს,
...
ჩანაწერები ერთად მუშვდება. Rust-ის typa სისტემა აქ
